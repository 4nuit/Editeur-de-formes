    ...
    implements Originator<Figure>, ListChangeListener<Figure>
{
	...
	public Figure initiateFigure(double x, double y)
	{
		 ...
		 */
		return figureType.getFigure(getFillColor(), getEdgeColor(), getLineType(), size(), logger, x, y);
	}
	...
	public Figure fromShape(Shape shape)
	{
		 ...
		 */
		ObservableList<Node> FIGS = root.getChildren();
		for (int i = 0; i < FIGS.size(); i++) {
			...
			if (n == shape) {
				...
				for (Node ni : ((Group) n).getChildren()) {
					if (ni == shape) {
						return figures.get(i);
					}
				}	
