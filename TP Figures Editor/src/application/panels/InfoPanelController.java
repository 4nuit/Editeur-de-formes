    ...
    implements Initializable
{
	...
	private void fillInfoPanel(Figure figure)
	{
		...
		translationXLabel.setText(String.format("%4.0f", figure.getRoot().getTranslateX()));
		translationYLabel.setText(String.format("%4.0f", figure.getRoot().getTranslateY()));
		rotationLabel.setText(String.format("%4.0f", figure.getRoot().getRotate()));		
		scaleLabel.setText(String.format("%4.0f", figure.getRoot().scaleXProperty().get()));
		fillColorCircle.setFill(figure.getFillColor());
		edgeColorCircle.setFill(null);
		figureTypeImageView.setImage(FigureIconsFactory.getIconFromType(FigureType.fromFigure(figure)));
	}
