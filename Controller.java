...
//		lineTypeCombobox.setValue(LineType.SOLID);
		 ...
		 */
        if (useFillColor.isSelected()==false) {
        	fillColorPicker.setDisable(true);
        	useEdgeColor.setSelected(true);
        }
        if (useEdgeColor.isSelected()==false) {
		 ...
		 * 	- disableProperty
		 */
        bindProperties(filteringProperty, filterToggleButton.selectedProperty());
        bindProperties(filteringProperty, filterToggleCheckMenuItem.selectedProperty());
        bindProperties(filterToggleButton.disableProperty(), filterToggleCheckMenuItem.disableProperty());
	...
	public void onEditAction(ActionEvent event)
	{
		 ...
		 */
		if (editToggleButton.isSelected() == false) {
	...
	public void onDeleteSelectedAction(ActionEvent event)
	{
		 ...
		 */
		ObservableList<Figure> selectedItems = figuresListView.getSelectionModel().getSelectedItems();
		int size = selectedItems.size();
		for (int i = size - 1; i >= 0; i--) {
			drawingModel.remove(selectedItems.get(i));
		}
	}
	...
	public void onApplyStyleAction(ActionEvent event)
	{
		...
		for (Figure elt : drawingModel) {
			if (elt.isSelected()) {
				elt.setFillColor(drawingModel.getFillColor());
				elt.setEdgeColor(drawingModel.getEdgeColor());
				elt.setLineType(drawingModel.getLineType());
				elt.setLineWidth(drawingModel.getLineWidth());
			}
		}
	}
	...
	public void onDisplayButtonsWithGraphicsOnlyAction(ActionEvent event)
	{
		 ...
		 */
		for (Labeled elt : styleableButtons){
			elt.setContentDisplay(ContentDisplay.GRAPHIC_ONLY);
		}
	}
	...
	public void onDisplayButtonsWithTextAndGraphicsAction(ActionEvent event)
	{
		 ...
		 */
		for (Labeled elt : styleableButtons){
			elt.setContentDisplay(ContentDisplay.LEFT);
		}
	...
	public void onDisplayButtonsWithTextOnlyAction(ActionEvent event)
	{
		 ...
		 */
		for (Labeled elt : styleableButtons){
			elt.setContentDisplay(ContentDisplay.TEXT_ONLY);
		}
	}
